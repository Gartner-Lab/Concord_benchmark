...
new_X = sp.csr_matrix(adata.X)
adata = ad.AnnData(X=new_X, obs=adata.obs, var=adata.var, uns=adata.uns, obsm=adata.obsm, varm=adata.varm)

output_key = 'Scanorama'

with timer:
    ccd.ul.run_scanorama(adata, batch_key="dataset3", output_key=output_key, dimred=300, return_corrected=False)
